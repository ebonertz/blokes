    ...
    return new Promise((function(_this) {
      return function(resolve, reject) {
        ...
        _processPage = function(lastId, acc) {
          var enhancedQueryString, queryString, wherePredicate;
          if (acc == null) {
            acc = [];
          }
          debug('processing next page with id: %s', lastId);
          _this._params.query = originalQuery;
          queryString = _this._queryString();
          if (lastId) {
            wherePredicate = encodeURIComponent("id > \"" + lastId + "\"");
          }
          debug('process where predicate: %j', wherePredicate);
          enhancedQueryString = ["sort=" + (encodeURIComponent("id asc")), queryString, "withTotal=false", wherePredicate ? "where=" + wherePredicate : null].filter(function(el) {
            return !!el;
          }).join('&');
