var BaseUtils, ProductUtils, REGEX_NUMBER, REGEX_UNDERSCORE_NUMBER, _, actionsBaseList, allVariants, debug,
...
ProductUtils = (function(superClass) {
  extend(ProductUtils, superClass);
  ...
  ProductUtils.prototype.diff = function(old_obj, new_obj) {
    ...
    patch = function(obj, arrayIndexFieldName) {
      ...
      return _.each(_allVariants, function(variant, index) {
        if (variant == null) {
          return variant;
        }
        patchPrices(variant);
  ...
  ProductUtils.prototype.actionsMapBase = function(diff, old_obj) {
    var actions;
    actions = [];
    _.each(actionsBaseList(), (function(_this) {
      return function(item) {
        var action, orderHintActions;
        action = _this._buildBaseAttributesAction(item, diff, old_obj);
        if ((action != null ? action.action : void 0) === "setCategoryOrderHint") {
          if (action.categoryOrderHints === void 0) {
            return;
          }
          orderHintActions = Object.keys(action.categoryOrderHints).map(function(categoryId) {
            var orderHint;
            orderHint = action.categoryOrderHints[categoryId];
            if (orderHint === null || orderHint === void 0) {
              orderHint = void 0;
            } else {
              orderHint = ("" + orderHint) || void 0;
            }
            return {
          ...
          return actions = actions.concat(orderHintActions);
        } else {
          if (action) {
    ...
    if (diff.state) {
      if (_.isArray(diff.state.id)) {
        action = {
          ...
          state: {
            typeId: 'state',
            id: this.getDeltaValue(diff.state.id)
          }
        };
        actions.push(action);
      } else {
  ...
  ProductUtils.prototype.actionsMapCategories = function(diff) {
    var actions;
    actions = [];
    if (diff.categories) {
  ...
  ProductUtils.prototype.actionsMapPrices = function(diff, old_obj, new_obj) {
    var _mapVariantPrices, actions, prices;
    actions = [];
      ...
      return function(price, key, old_variant, new_variant) {
        ...
        if (index) {
          ...
          if (_.size(price) === 1 && _.size(price.value) === 1 && _.has(price.value, 'centAmount')) {
            ...
            addAction = _this._buildAddPriceAction(old_variant, new_variant, index);
            if (addAction) {
    ...
    if (obj) {
      ...
      if (_.isArray(obj)) {
        ...
        _.each(keys, (function(_this) {
          return function(k) {
            var diffed_keys, diffed_keywords, diffed_value, value;
            diffed_value = obj[k];
            if (_.isArray(diffed_value)) {
              value = _this.getDeltaValue(diffed_value, old_obj[key][k]);
              if (!_.find(value, function(val) {
                return _.has(val, 'text') && val['text'] === "";
              })) {
              ...
              if (_.has(diffed_value, '_t') && diffed_value['_t'] === 'a') {
                ...
                _.each(diffed_keys, function(v) {
                  if (REGEX_NUMBER.test(v)) {
                    return diffed_keywords.push(diffed_value[v]);
                  }
                });
                diffed_keywords = _.flatten(diffed_keywords);
                if (!_.isEqual(diffed_keywords, old_obj[key][k])) {
                  return updated[k] = diffed_keywords;
                }
              }
            } else {
              return updated[k] = diffed_value;
            }
          };
        })(this));
      }
      ...
      if (_.isEmpty(updated) && key === "searchKeywords") {
        action = void 0;
      }
    }
    return action;
  ...
  ProductUtils.prototype._buildAddPriceAction = function(old_variant, new_variant, index) {
    var action, price;
      ...
      return function(image, key) {
        ...
        if (REGEX_NUMBER.test(key)) {
          if (_.isArray(image) && image.length) {
            return actions.push(_this._buildAddExternalImageAction(old_variant, new_variant.images[key]));
          } else if (_.isObject(image)) {
            if (_.has(image, 'url') && image.url.length === 2) {
              actions.push(_this._buildRemoveImageAction(old_variant, old_variant.images[key]));
              return actions.push(_this._buildAddExternalImageAction(old_variant, new_variant.images[key]));
            } else if (_.has(image, 'label') && (image.label.length === 1 || image.label.length === 2)) {
              return actions.push({
  ...
  ProductUtils.prototype._buildSetAttributeAction = function(diffed_value, old_variant, attribute, sameForAllAttributeNames) {
    ...
    if (attribute) {
      ...
      if (_.isArray(diffed_value)) {
        action.value = this.getDeltaValue(diffed_value, attribute.value);
      } else {
          ...
          if (_.has(diffed_value, '_t') && diffed_value['_t'] === 'a') {
            ...
            _.each(diffed_value, (function(_this) {
              return function(localValue, lang) {
                return text[lang] = _this.getDeltaValue(localValue, attrib.value[lang]);
              };
            })(this));
            action.value = text;
          }
  ...
  ProductUtils.prototype._buildNewSetAttributeAction = function(id, el, sameForAllAttributeNames) {
    ...
    if (_.isArray(action.value)) {
      _.each(action.value, function(v) {
        if (!_.isString(v)) {
          return delete v._MATCH_CRITERIA;
        }
      });
...
allVariants = function(product) {
  var masterVariant, ref, variants;
  ref = _.defaults(product, {
    masterVariant: void 0,
    variants: []
  }), masterVariant = ref.masterVariant, variants = ref.variants;
